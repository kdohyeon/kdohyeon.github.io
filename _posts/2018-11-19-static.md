...
## 1. Static 이란?
* 프로그래밍 용어로써 Static 은 정적, 고정의 뜻을 가지고 있음
* 프로그래밍 언어마다 조금씩은 다른 의미를 가지고 있으며 Java의 경우, static 이 어떻게 활용되는지 알아보고자 함
...
하나의 예시를 보면서 설명을 하고자 함
* 예시는 총 52개의 Playing Card 를 구현하기 위해 Card 클래스를 선언함
+ 클래스 내 데이터(i.e., 변수, 함수)로는 카드의 무늬, 숫자, 폭, 높이를 포함함
+ 카드의 무늬와 숫자는 인스턴스마다 다른 값을 유지해야 하므로 인스턴스 변수로 설정하고, 카드의 폭과 높이는 모든 인스턴스에 동일하게 적용되는 부분이므로 클래스 변수로 설정함
...
* 클래스 변수의 경우 Class 인스턴스의 선언없이 사용 가능함
* Card.width, redCard.width, blueCard.width 모두 같은 값을 공유함
    * 하나의 값이 바뀌면 다른 값도 모두 바뀜 --> 같은 공간을 참조(pointing)함
    * 예시에서처럼 redCard의 attribute를 바꿨는데도 blueCard의 attribute 값도 바뀜

## 3. 클래스 메소드와 인스턴스 메소드
* 호출을 할 때에도 클래스 변수처럼 인스턴스 생성 필요없이 바로 활용할 수 있음
    * Class.Method
* 어느 경우에 static 을 활용하여 메소드를 정의하는가?
    * 클래스 (class)는 변수(variable)와 함수(method)의 집합체임
    * 인스턴스 메소드는 해당 메소드를 수행하기 위해서 인스턴스 변수를 활용함
    * 따라서 어떤 메소드에서 인스턴스 변수를 활용하지 않거나, 클래스 변수 또는 매개 변수로 메소드를 활용한다면 클래스 메소드로 정의함
    * 인스턴스 변수를 활용하지 않는다고 해서 클래스 메소드로 무조건 정의하는것은 아니지만 일반적으론 그렇게 함
    * 예시는 Math 클래스가 있음
        * 모든 작업을 매개 변수로만 진행함

## 4. 클래스 멤버와 인스턴스 멤버간 호출 및 참조
* 같은 클래스 내 멤버들간에는 별도의 인스턴스를 생성하지 않고도 서로 호출 및 참조 가능함
* 단, 클래스 멤버가 인스턴스 멤버를 호출 및 참조하고자 하는 경우에는 인스턴스를 생성해야 함
    * 이유는 인스턴스 멤버가 클래스 멤버를 호출하는 시점에는 클래스 멤버의 존재가 보장되지만
    * 클래스 멤버가 인스턴스 멤버를 호출하는 시점에는 인스턴스 멤버의 존재가 보장되지 않음
```Java
/* Example */
class MemberCall{
...
```
* 클래스 멤버 (i.e., 변수, 메소드)는 언제나 호출 및 참조 가능함
* 따라서, 클래스 멤버 > 클래스 멤버, 인스턴스 멤버 > 클래스 멤버는 언제나 가능함
* 더불어, 인스턴스 멤버 > 인스턴스 멤버 역시 언제나 가능함
* 하지만, 인스턴스 멤버 > 클래스 멤버는 클래스 멤버의 인스턴스 생성이 필요함
